...
public class ResourceEvent extends Event {
	 ...
	 */
	public ResourceEvent(LocationOnRoad pickupLoc, LocationOnRoad dropoffLoc, long availableTime, Simulator simulator, Double fare) {
		super(availableTime, simulator);
		this.pickupLoc = pickupLoc;
		this.dropoffLoc = dropoffLoc;
		this.eventType = EventType.Resource;
	...
	public List<Pair<AgentEvent, Long>> getAgentsAndArrivalTime() {

		List<Pair<AgentEvent, Long>> agentArrivalTimePairList = new ArrayList<>();
		for (AgentEvent agent : simulator.emptyAgents) {
			...
			if (arriveTime <= availableTime + simulator.ResourceMaximumLifeTime){
 				agentArrivalTimePairList.add(new Pair<>(agent, travelTime));
			}
		}
		return agentArrivalTimePairList;
	}


	public AgentEvent assignAgentToResource(AgentEvent bestAgent,long earliest ){

		long travelTimeFromStartIntersection = bestAgent.loc.road.travelTime - (bestAgent.time - time);
		...
		long approachTime = earliest;
		long searchTime = cruiseTime + approachTime;
		long waitTime = earliest - availableTime + time;
		...
		bestAgent.assignedTo(agentLocationOnRoad, time, id, pickupLoc, dropoffLoc);
		...
		bestAgent.setEvent(time + earliest + tripTime, dropoffLoc, AgentEvent.DROPPING_OFF);
