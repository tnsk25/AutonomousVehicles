	...
	public ArrayList<Resource> parse() {
		...
		try {
			...
			while (sc.hasNext()) {
				...
				while(i-- >0)
					sc.next();
				double fare = Double.parseDouble(sc.next().replace("\r",""));
				...
				if(fare <= 1)
					continue;
				resources.add(new Resource(pickupLat, pickupLon, dropoffLat, dropoffLon, time, fare)); //create new resource with the above fields
			}
			sc.close();
		} catch (Exception e) {
